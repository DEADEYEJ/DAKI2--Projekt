...
class BasePreprocessor:
    ...
    def parse_annotations(self, annotation_obj: Any) -> Tuple[List[EntitySpan], List[str]]:
        issues = []
        ...
        for idx, item in enumerate(parsed):
            ...
            entities.append(
                EntitySpan(start=start, end=end, label=str(label), value=value)
            )
    ...
    def process_sample(self, sample_id: Any, raw_text: Any, raw_annotations: Any) -> ProcessedSample:
        ...
        entities, parse_issues = self.parse_annotations(raw_annotations)
...
class SequenceProcessor:
    ...
    def add_sequence_info(
        ...
        for sample in samples:
            tokens = self.tokenize_with_offsets(sample.clean_text)
...
def create_dev_test_split(
    ...
    """
    Laver ét fast hold-out test-sæt.
    Dette test-sæt må ikke bruges under modeludvikling.
    """
...
def make_cv_splits(
    ...
    """
    K-fold CV på udviklingssættet.
    Med 9 folds på 90% udviklingsdata får du ca.:
    - 80% træning
    - 10% validering
    - 10% hold-out test
    """
...
def evaluate_labels(y_true: List[str], y_pred: List[str]) -> Dict[str, Any]:
    ...
    eval_labels = [lbl for lbl in labels if lbl != "O"] if "O" in labels else labels
...
def evaluate_on_test_if_enabled(
    ...
    """
    Test evalueres kun hvis enabled=True.
...
if __name__ == "__main__":
    ...
    # 2) Lav CV-folds på dev
    # cv_folds = make_cv_splits(dev_df, n_splits=9, random_state=42)
    ...
    # 4) Kun valideringsresultater under udvikling
    # cv_results = []
    #
    # for fold_no, (train_fold_df, val_fold_df) in enumerate(cv_folds, start=1):
    #     train_samples = base.process_dataframe(train_fold_df)
    #     val_samples = base.process_dataframe(val_fold_df)
    #
    #     # Eksempel: CRF pipeline
    #     seq = SequenceProcessor()
    #     train_seq = seq.add_sequence_info(train_samples, tagging_scheme="BIO")
    #     val_seq = seq.add_sequence_info(val_samples, tagging_scheme="BIO")
    #
    #     crf_adapter = CRFAdapter()
    #     X_train = crf_adapter.to_features(train_seq)
    #     y_train = crf_adapter.to_labels(train_seq)
    #     X_val = crf_adapter.to_features(val_seq)
    #     y_val = crf_adapter.to_labels(val_seq)
    #
    #     # HER skal du indsætte din modeltræning og prediction
    #     # model.fit(X_train, y_train)
    #     # y_val_pred = model.predict(X_val)
    #
    #     # flatten til samlet evaluering
    #     # y_true_flat = flatten_label_sequences(y_val)
    #     # y_pred_flat = flatten_label_sequences(y_val_pred)
    #
    #     # fold_metrics = evaluate_labels(y_true_flat, y_pred_flat)
    #     # cv_results.append(fold_metrics)
    #
    #     # print(f"Fold {fold_no} macro-F1: {fold_metrics['macro']['f1']:.4f}")

    # 5) Samlet valideringsresume
    # summary = summarize_cv_results(cv_results)
    # print(summary)

    # 6) Test forbliver skjult indtil aktivering
    # ACTIVATE_TEST = False
    #
    # if ACTIVATE_TEST:
    #     # preprocess hele dev + test
    #     dev_samples = base.process_dataframe(dev_df)
    #     test_samples = base.process_dataframe(test_df)
    #
    #     # træn endelig model på hele dev
    #     # evaluer én gang på test
    #     pass
